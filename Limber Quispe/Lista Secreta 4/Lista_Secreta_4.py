...
# Paso único: Ingreso de todos los valores en una línea separada por comas
while True:
    try:
        try:
            entrada_oculta = getpass.getpass("🔒 Ingrese los números secretos separados por comas (oculto): ")
        except Exception:
            print("⚠️ Entrada oculta no compatible en este entorno. Usando entrada visible.")
        ...
        if not lista_secreta:
            print("⚠️ La lista no puede estar vacía.")
        else:
            break
    except ValueError:
        print("⚠️ Error: asegúrate de ingresar solo números enteros separados por comas.")
